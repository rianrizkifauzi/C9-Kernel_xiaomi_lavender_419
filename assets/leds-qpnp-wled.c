...
static int qpnp_wled_probe(struct platform_device *pdev)
{
	...
	/* C9 DEBUG: force backlight on at probe to validate qpnp-wled HW path */
	wled->cdev.brightness = wled->cdev.max_brightness / 2;
	pr_info("C9DWN: probe force-on br=%d max=%d\n",
		 wled->cdev.brightness, wled->cdev.max_brightness);
	queue_work(wled->wq, &wled->work);
